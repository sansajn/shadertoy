...
float signed_distance(in vec3 p)
{
	...
	return length(p) - (sphere_radius + displacement);
}

bool sphere_trace(in vec3 orig, in vec3 dir, out vec3 pos)
{
	if (dot(orig, orig) - pow(dot(orig, dir), 2.0) > pow(sphere_radius, 2.0)) 
...
vec3 distance_field_normal(in vec3 pos)
{
	...
	return normalize(vec3(
		signed_distance(pos + vec3(eps, 0.0, 0.0)) - d,
		signed_distance(pos + vec3(0.0, eps, 0.0)) - d,
		signed_distance(pos + vec3(0.0, 0.0, eps)) - d
	));
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
	...
	vec3 hit, col;
	if (sphere_trace(vec3(0.0, 0.0, 3.0), dir, hit))
	{
